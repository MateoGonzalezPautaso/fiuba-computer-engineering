...
sys_exofork(void)
{
	...
	newenv->env_priority =
	        curenv->env_priority;  // El hijo hereda la prioridad del padre
...
// Returns the current environment's priority.
static uint32_t
...
/* Reduce la prioridad times_dropped veces.
 * En caso de que la prioridad reducida sea menor a la minima, el proceso
 * resulta con la prioridad minima.
 */
static int
sys_droppriority(envid_t env_id, uint32_t times_dropped)
{
	struct Env *env;
	int r;
	...
	if ((r = envid2env(env_id, &env, 1)) < 0)
		return r;

	uint32_t dropped_priority = env->env_priority + times_dropped;
	env->env_priority = MIN(LOWEST_PRIORITY, dropped_priority);
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_droppriority:
		return sys_droppriority(a1, a2);
	case SYS_yield:
		sys_yield();  // No return
	default:
		return -E_INVAL;
